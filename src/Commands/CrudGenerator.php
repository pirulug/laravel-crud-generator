...
class CrudGenerator extends GeneratorCommand
{
  ...
  public function handle()
  {
    ...
    if (!$this->tableExists()) {
      $this->error("`{$this->table}` table does not exist in the database.");
      return false;
    }
  ...
  protected function writeRoute(): static
  {
    ...
    $this->info('Please add the following route to your `web.php` or `api.php`:');
    $this->info('');
  ...
  protected function buildController(): static
  {
    ...
    if ($this->files->exists($controllerPath) && $this->ask("Controller already exists. Overwrite? (y/n)", 'y') == 'n') {
      return $this;
    }
  ...
  protected function buildModel(): static
  {
    ...
    if ($this->files->exists($modelPath) && $this->ask("Model already exists. Overwrite? (y/n)", 'y') == 'n') {
      return $this;
    }
  ...
  protected function buildViews(): static
  {
    ...
    foreach ($this->getFilteredColumns() as $column) {
      ...
      $resourceFields .= $this->getResourceField($column);
    }
    ...
    foreach (['index', 'create', 'edit', 'form', 'show'] as $view) {
      $path = $this->isCustomStubFolder()
        ...
        : match ($this->options['stack']) {
          'livewire' => $this->isModernLaravel() ? "views/{$this->options['stack']}/12/$view" : "views/{$this->options['stack']}/default/$view",
