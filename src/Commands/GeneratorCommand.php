...
namespace Pirulug\CrudGenerator\Commands;

use Exception;
use Pirulug\CrudGenerator\ModelGenerator;
...
abstract class GeneratorCommand extends Command implements PromptsForMissingInput
{
   ...
   * @return bool
   */
  public function isModernLaravel(): bool
  {
    return (int) app()->version() >= 12;
  }
  ...
  protected function getField(string $title, string $column, string $type = 'form-field'): string
  {
    ...
    $path = match ($this->options['stack']) {
      'livewire' => $this->isModernLaravel() ? "views/{$this->options['stack']}/12/$type" : "views/{$this->options['stack']}/default/$type",
   ...
   * @return string
   */
  protected function getResourceField($column): string
  {
    return $this->_getSpace(3) . "'{$column}' => \$this->{$column},\n";
  }
  ...
  protected function buildLayout(): void
  {
    ...
    $uiPackage = match ($this->options['stack']) {
      ...
      'livewire' => $this->isModernLaravel() ? 'laravel/livewire-starter-kit' : 'laravel/breeze',
      default => 'laravel/ui'
    };
    ...
    // Do not run command for v12.* or higher
  ...
  protected function modelReplacements(): array
  {
    ...
    foreach ($this->getColumns() as $column) {
      ...
      if (Str::contains($type, ['text', 'varchar', 'string'])) {
        $rulesArray[$column['name']][] = 'string';
      }

      if (Str::contains($type, ['int', 'integer', 'bigint', 'smallint', 'tinyint'])) {
        $rulesArray[$column['name']][] = 'integer';
      }

      if (Str::contains($type, ['decimal', 'float', 'double', 'numeric'])) {
        $rulesArray[$column['name']][] = 'numeric';
      }

      if (Str::contains($type, ['date', 'datetime', 'timestamp'])) {
        $rulesArray[$column['name']][] = 'date';
      }

      if ($column['name'] == 'email') {
        $rulesArray[$column['name']][] = 'email';
      }
